...
def invert_series(df: pd.DataFrame, channels, dbb_ft: float, dip_app_deg: float, mode='below'):
    ...
    for i in range(len(df)):
        ...
        if not ok:
            out_rows.append({'MD': md[i], 'INC': float(inc[i]), 'DTB_Target_Sensor_ft': np.nan, 'DTB_Target_Bit_ft': np.nan,
                             'R0_ohm_m': np.nan, 'Rsh_ohm_m': np.nan, 'Misfit': np.nan, 'Confidence': 0.0, 'Mode': mode})
            continue
        ...
        out_rows.append({'MD': md[i], 'INC': float(inc[i]), 'DTB_Target_Sensor_ft': float(h), 'DTB_Target_Bit_ft': float(h_bit),
                         'R0_ohm_m': float(R0), 'Rsh_ohm_m': float(Rsh), 'Misfit': float(chi2), 'Confidence': conf, 'Mode': mode})
